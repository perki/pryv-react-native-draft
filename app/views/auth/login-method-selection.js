...
const LoginMethodSelection = ({ navigation, route }) => {
  ...
  class MyLoginButton {
    ...
    async getAuthorizationData () {
      ...
      const authData = await storage.getItemAsync(this._cookieKey);
      return JSON.parse(authData);
    }

    saveAuthorizationData (authData) {
      storage.setItemAsync(this._cookieKey, JSON.stringify(authData));
    ...
    async deleteAuthorizationData () {
      ...
      if (await storage.isAvailableAsync(this._cookieKey)) {
        await storage.deleteItemAsync(this._cookieKey);
      }
    }
  }
